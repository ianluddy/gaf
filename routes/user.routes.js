...
router.get("/user", jwt.verificationHandler(), async (ctx) => {
  ctx.body = await userService.find();
  ctx.status = HttpStatus.OK;
});

router.post("/user", jwt.verificationHandler(), async (ctx) => {
  await userService.insertOne(ctx.request.body);
  ctx.status = HttpStatus.CREATED;
});

router.get("/user/:id", jwt.verificationHandler(), async (ctx) => {
  const query = {"_id": ObjectID(ctx.params.id)};
  ctx.body = await userService.findOne(query);
  ctx.status = HttpStatus.OK;
});

router.put("/user/:id", jwt.verificationHandler(), async (ctx) => {
...
router.delete("/user/:id", jwt.verificationHandler(), async (ctx) => {
  const query = {"id": ObjectID(ctx.params.id)};
